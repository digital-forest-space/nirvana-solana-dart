...
/// Withdraw navToken from a Mayflower market's personal position escrow
///
/// Usage: dart scripts/samsara/withdraw_nav.dart <keypair_path> <amount> [--market <name>] [--rpc <url>] [--verbose] [--dry-run]
///
/// Examples:
///   dart scripts/samsara/withdraw_nav.dart ~/.config/solana/id.json 1.0
///   dart scripts/samsara/withdraw_nav.dart ~/.config/solana/id.json 0.5 --market navSOL --verbose
///   dart scripts/samsara/withdraw_nav.dart ~/.config/solana/id.json 0.1 --dry-run
///
/// Environment:
...
void main(List<String> args) async {
  if (args.length < 2) {
    print('Usage: dart scripts/samsara/withdraw_nav.dart <keypair_path> <amount> [options]');
    print('');
    print('Withdraws navToken (e.g., navSOL) from the market escrow back to your wallet.');
    print('');
    print('Options:');
    print('  --market <name>  Market to withdraw from (default: navSOL)');
  ...
  if (verbose) {
    print('\nWithdraw:');
    print('  Amount: $amount ${market.name} ($lamports lamports)');
  ...
  final unsignedTxBytes = await client.buildUnsignedWithdrawNavTokenTransaction(
    userPubkey: userPubkey,
    market: market,
    withdrawLamports: lamports,
