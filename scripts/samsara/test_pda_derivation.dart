...
void main() async {
  ...
  LogService.log('Testing PDA derivations...\n');
  LogService.log('User 1: YOUR_WALLET_ADDRESS_HERE');
  LogService.log('Expected position: $expectedPosition1');
  LogService.log('Expected shares: $expectedShares1\n');
  ...
  LogService.log('=== Testing personal_position PDA ===\n');
    ...
    for (final entry in marketAccounts.entries) {
      try {
        ...
        if (pda.toBase58() == expectedPosition1) {
          LogService.log('✅ MATCH! Seeds: ["$prefix", user, ${entry.key}]');
          LogService.log('   Result: ${pda.toBase58()}\n');
        }
      } catch (_) {}
    }
  }
  ...
  for (final prefix in prefixes) {
    for (final entry in marketAccounts.entries) {
      try {
        ...
        final pda = await Ed25519HDPublicKey.findProgramAddress(seeds: seeds, programId: mayflowerProgram);
        if (pda.toBase58() == expectedPosition1) {
          LogService.log('✅ MATCH! Seeds: ["$prefix", ${entry.key}, user]');
          LogService.log('   Result: ${pda.toBase58()}\n');
        }
      } catch (_) {}
    }
  ...
  for (final entry in marketAccounts.entries) {
    try {
      ...
      final pda = await Ed25519HDPublicKey.findProgramAddress(seeds: seeds, programId: mayflowerProgram);
      if (pda.toBase58() == expectedPosition1) {
        LogService.log('✅ MATCH! Seeds: [user, ${entry.key}]');
        LogService.log('   Result: ${pda.toBase58()}\n');
      }
    } catch (_) {}
  }
  ...
  for (final entry in marketAccounts.entries) {
    try {
      ...
      final pda = await Ed25519HDPublicKey.findProgramAddress(seeds: seeds, programId: mayflowerProgram);
      if (pda.toBase58() == expectedPosition1) {
        LogService.log('✅ MATCH! Seeds: [${entry.key}, user]');
        LogService.log('   Result: ${pda.toBase58()}\n');
      }
    } catch (_) {}
    ...
    try {
      ...
      final pda = await Ed25519HDPublicKey.findProgramAddress(seeds: seeds, programId: mayflowerProgram);
      if (pda.toBase58() == expectedPosition1) {
        LogService.log('✅ MATCH! Seeds: [${entry.key}, user, navMint]');
        LogService.log('   Result: ${pda.toBase58()}\n');
      }
    } catch (_) {}
  }
  ...
  for (final prefix in sharesPrefixes) {
    for (final entry in marketAccounts.entries) {
      ...
      try {
        ...
        final pda = await Ed25519HDPublicKey.findProgramAddress(seeds: seeds, programId: mayflowerProgram);
        if (pda.toBase58() == expectedShares1) {
          LogService.log('✅ MATCH! Seeds: ["$prefix", user, ${entry.key}, navMint]');
          LogService.log('   Result: ${pda.toBase58()}\n');
        }
      } catch (_) {}
    }
  ...
  for (final entry in marketAccounts.entries) {
    try {
      ...
      final pda = await Ed25519HDPublicKey.findProgramAddress(seeds: seeds, programId: mayflowerProgram);
      if (pda.toBase58() == expectedShares1) {
        LogService.log('✅ MATCH! Seeds: [user, ${entry.key}, navMint]');
        LogService.log('   Result: ${pda.toBase58()}\n');
      }
    } catch (_) {}
  }
  ...
  try {
    ...
    final pda = await Ed25519HDPublicKey.findProgramAddress(seeds: seeds, programId: mayflowerProgram);
    if (pda.toBase58() == expectedShares1) {
      LogService.log('✅ MATCH! Seeds: [personal_position, navMint]');
      LogService.log('   Result: ${pda.toBase58()}\n');
    }
  } catch (_) {}
  ...
  for (final prefix in sharesPrefixes) {
    try {
      ...
      final pda = await Ed25519HDPublicKey.findProgramAddress(seeds: seeds, programId: mayflowerProgram);
      if (pda.toBase58() == expectedShares1) {
        LogService.log('✅ MATCH! Seeds: ["$prefix", personal_position]');
        LogService.log('   Result: ${pda.toBase58()}\n');
      }
    } catch (_) {}
    ...
    try {
      ...
      final pda = await Ed25519HDPublicKey.findProgramAddress(seeds: seeds, programId: mayflowerProgram);
      if (pda.toBase58() == expectedShares1) {
        LogService.log('✅ MATCH! Seeds: ["$prefix", personal_position, navMint]');
        LogService.log('   Result: ${pda.toBase58()}\n');
      }
    } catch (_) {}
  }
  ...
  for (final prefix in sharesPrefixes) {
    try {
      ...
      final pda = await Ed25519HDPublicKey.findProgramAddress(seeds: seeds, programId: mayflowerProgram);
      if (pda.toBase58() == expectedShares1) {
        LogService.log('✅ MATCH! Seeds: ["$prefix", marketMetadata, user]');
        LogService.log('   Result: ${pda.toBase58()}\n');
      }
    } catch (_) {}
  }
  ...
  for (final prefix in sharesPrefixes) {
    try {
      ...
      final pda = await Ed25519HDPublicKey.findProgramAddress(seeds: seeds, programId: mayflowerProgram);
      if (pda.toBase58() == expectedShares1) {
        LogService.log('✅ MATCH! Seeds: ["$prefix", navMint, user]');
        LogService.log('   Result: ${pda.toBase58()}\n');
      }
    } catch (_) {}
    ...
    try {
      ...
      final pda = await Ed25519HDPublicKey.findProgramAddress(seeds: seeds, programId: mayflowerProgram);
      if (pda.toBase58() == expectedShares1) {
        LogService.log('✅ MATCH! Seeds: ["$prefix", user, navMint]');
        LogService.log('   Result: ${pda.toBase58()}\n');
      }
    } catch (_) {}
  }
  ...
  try {
    ...
    final pda = await Ed25519HDPublicKey.findProgramAddress(seeds: seeds, programId: mayflowerProgram);
    if (pda.toBase58() == expectedShares1) {
      LogService.log('✅ MATCH! Seeds: [navMint, user]');
      LogService.log('   Result: ${pda.toBase58()}\n');
    }
  } catch (_) {}
  ...
  try {
    ...
    final pda = await Ed25519HDPublicKey.findProgramAddress(seeds: seeds, programId: mayflowerProgram);
    if (pda.toBase58() == expectedShares1) {
      LogService.log('✅ MATCH! Seeds: [user, navMint]');
      LogService.log('   Result: ${pda.toBase58()}\n');
    }
  } catch (_) {}
  ...
  try {
    ...
    final pda = await Ed25519HDPublicKey.findProgramAddress(seeds: seeds, programId: mayflowerProgram);
    if (pda.toBase58() == expectedShares1) {
      LogService.log('✅ MATCH! Seeds: [marketMetadata, navMint, user]');
      LogService.log('   Result: ${pda.toBase58()}\n');
    }
  } catch (_) {}
  ...
  try {
    ...
    if (pda.toBase58() == expectedShares1) {
      LogService.log('✅ MATCH! ATA Seeds: [user, tokenProgram, navMint] with ATA program');
      LogService.log('   Result: ${pda.toBase58()}\n');
    }
  } catch (_) {}
  ...
  try {
    ...
    final pda = await Ed25519HDPublicKey.findProgramAddress(seeds: seeds, programId: mayflowerProgram);
    if (pda.toBase58() == expectedShares1) {
      LogService.log('✅ MATCH! Seeds: [user, authority, navMint]');
      LogService.log('   Result: ${pda.toBase58()}\n');
    }
  } catch (_) {}
  ...
  for (final prefix in sharesPrefixes) {
    try {
      ...
      if (pda.toBase58() == expectedShares1) {
        LogService.log('✅ MATCH! Samsara Seeds: ["$prefix", marketMetadata, user]');
        LogService.log('   Result: ${pda.toBase58()}\n');
      }
    } catch (_) {}
    ...
    try {
      ...
      final pda = await Ed25519HDPublicKey.findProgramAddress(seeds: seeds, programId: samsaraProgram);
      if (pda.toBase58() == expectedShares1) {
        LogService.log('✅ MATCH! Samsara Seeds: ["$prefix", navMint, user]');
        LogService.log('   Result: ${pda.toBase58()}\n');
      }
    } catch (_) {}
    ...
    try {
      ...
      final pda = await Ed25519HDPublicKey.findProgramAddress(seeds: seeds, programId: samsaraProgram);
      if (pda.toBase58() == expectedShares1) {
        LogService.log('✅ MATCH! Samsara Seeds: ["$prefix", user, marketMetadata]');
        LogService.log('   Result: ${pda.toBase58()}\n');
      }
    } catch (_) {}
  }
  ...
  for (final prefix in sharesPrefixes) {
    try {
      ...
      final pda = await Ed25519HDPublicKey.findProgramAddress(seeds: seeds, programId: mayflowerProgram);
      if (pda.toBase58() == expectedShares1) {
        LogService.log('✅ MATCH! Seeds: ["$prefix", mayflowerMarket, user]');
        LogService.log('   Result: ${pda.toBase58()}\n');
      }
    } catch (_) {}
    ...
    try {
      ...
      final pda = await Ed25519HDPublicKey.findProgramAddress(seeds: seeds, programId: mayflowerProgram);
      if (pda.toBase58() == expectedShares1) {
        LogService.log('✅ MATCH! Seeds: ["$prefix", user, mayflowerMarket]');
        LogService.log('   Result: ${pda.toBase58()}\n');
      }
    } catch (_) {}
  }
  ...
  for (final prefix in ppPrefixes) {
    ...
    try {
      final seeds = [prefix.codeUnits, marketMetadata.bytes, user1.bytes];
      final pda = await Ed25519HDPublicKey.findProgramAddress(seeds: seeds, programId: mayflowerProgram);
      LogService.log('   Testing ["$prefix", marketMetadata, user] -> ${pda.toBase58()}');
      if (pda.toBase58() == expectedShares1) {
        LogService.log('✅ MATCH!');
      }
    } catch (e) {
      LogService.log('   Error with "$prefix": $e');
    }
  }
  ...
  try {
    ...
    final seeds = [user1.bytes, tokenProgram.bytes, navMint.bytes];
    final pda = await Ed25519HDPublicKey.findProgramAddress(seeds: seeds, programId: ataProgram);
    LogService.log('User1 navMint ATA: ${pda.toBase58()}');
    if (pda.toBase58() == expectedShares1) {
      LogService.log('✅ user_shares IS the navMint ATA!');
    } else {
      LogService.log('❌ Not a match. Expected: $expectedShares1');
    }
  } catch (e) {
    LogService.log('Error computing ATA: $e');
  }
  ...
  try {
    ...
    final pda = await Ed25519HDPublicKey.findProgramAddress(seeds: seeds, programId: ataProgram);
    LogService.log('personal_position navMint ATA: ${pda.toBase58()}');
    if (pda.toBase58() == expectedShares1) {
      LogService.log('✅ user_shares IS the ATA owned by personal_position!');
    } else {
      LogService.log('❌ Not a match. Expected: $expectedShares1');
    }
  } catch (e) {
  ...
  try {
    ...
    final pda = await Ed25519HDPublicKey.findProgramAddress(seeds: seeds, programId: ataProgram);
    LogService.log('authority navMint ATA: ${pda.toBase58()}');
    if (pda.toBase58() == expectedShares1) {
      LogService.log('✅ user_shares IS the ATA owned by authority!');
    }
  } catch (e) {
  ...
  for (int i = 0; i < 256; i++) {
    try {
      ...
      final pda = await Ed25519HDPublicKey.findProgramAddress(seeds: seeds, programId: mayflowerProgram);
      if (pda.toBase58() == expectedShares1) {
        LogService.log('✅ MATCH! Seeds: [[$i], marketMetadata, user]');
      }
    } catch (_) {}
    ...
    try {
      ...
      final pda = await Ed25519HDPublicKey.findProgramAddress(seeds: seeds, programId: mayflowerProgram);
      if (pda.toBase58() == expectedShares1) {
        LogService.log('✅ MATCH! Seeds: [[$i], user, marketMetadata]');
      }
    } catch (_) {}
    ...
    try {
      ...
      final pda = await Ed25519HDPublicKey.findProgramAddress(seeds: seeds, programId: mayflowerProgram);
      if (pda.toBase58() == expectedShares1) {
        LogService.log('✅ MATCH! Seeds: [[$i], navMint, user]');
      }
    } catch (_) {}
  }
  ...
  for (final suffix in ppSuffixes) {
    try {
      ...
      final pda = await Ed25519HDPublicKey.findProgramAddress(seeds: seeds, programId: mayflowerProgram);
      if (pda.toBase58() == expectedShares1) {
        LogService.log('✅ MATCH! Seeds: [personal_position, "$suffix"]');
      }
    } catch (_) {}
    ...
    try {
      ...
      final pda = await Ed25519HDPublicKey.findProgramAddress(seeds: seeds, programId: mayflowerProgram);
      if (pda.toBase58() == expectedShares1) {
        LogService.log('✅ MATCH! Seeds: ["$suffix", personal_position]');
      }
    } catch (_) {}
    ...
    try {
      ...
      final pda = await Ed25519HDPublicKey.findProgramAddress(seeds: seeds, programId: mayflowerProgram);
      if (pda.toBase58() == expectedShares1) {
        LogService.log('✅ MATCH! Seeds: [personal_position, "$suffix", navMint]');
      }
    } catch (_) {}
  }
  ...
  for (final prefix in mgPrefixes) {
    try {
      ...
      final pda = await Ed25519HDPublicKey.findProgramAddress(seeds: seeds, programId: mayflowerProgram);
      if (pda.toBase58() == expectedShares1) {
        LogService.log('✅ MATCH! Seeds: ["$prefix", marketGroup, user]');
      }
    } catch (_) {}
    ...
    try {
      ...
      final pda = await Ed25519HDPublicKey.findProgramAddress(seeds: seeds, programId: mayflowerProgram);
      if (pda.toBase58() == expectedShares1) {
        LogService.log('✅ MATCH! Seeds: ["$prefix", user, marketGroup]');
      }
    } catch (_) {}
  }
  ...
  try {
    ...
    final pda = await Ed25519HDPublicKey.findProgramAddress(seeds: seeds, programId: mayflowerProgram);
    LogService.log('Seeds [personal_position]: ${pda.toBase58()}');
    if (pda.toBase58() == expectedShares1) {
      LogService.log('✅ MATCH!');
    }
  } catch (_) {}
  ...
  try {
    ...
    final pda = await Ed25519HDPublicKey.findProgramAddress(seeds: seeds, programId: mayflowerProgram);
    LogService.log('Seeds [navMint]: ${pda.toBase58()}');
  } catch (_) {}
  ...
  try {
    ...
    final pda = await Ed25519HDPublicKey.findProgramAddress(seeds: seeds, programId: ataProgram);
    LogService.log('marketMetadata navMint ATA: ${pda.toBase58()}');
    if (pda.toBase58() == expectedShares1) {
      LogService.log('✅ MATCH!');
    }
  } catch (_) {}
  ...
  try {
    ...
    final pda = await Ed25519HDPublicKey.findProgramAddress(seeds: seeds, programId: ataProgram);
    LogService.log('marketGroup navMint ATA: ${pda.toBase58()}');
    if (pda.toBase58() == expectedShares1) {
      LogService.log('✅ MATCH!');
    }
  } catch (_) {}
  ...
  for (final seedList in tokenSeeds) {
    try {
      ...
      if (pda.toBase58() == expectedShares1) {
        LogService.log('✅ MATCH! Seeds: $seedList');
        LogService.log('   Result: ${pda.toBase58()}');
      }
    } catch (e) {
      // Skip errors
  ...
  try {
    final seeds = [pp1.bytes, tokenProgram.bytes, navMint.bytes];
    final pda = await Ed25519HDPublicKey.findProgramAddress(seeds: seeds, programId: mayflowerProgram);
    LogService.log('ATA-style with Mayflower: ${pda.toBase58()}');
    if (pda.toBase58() == expectedShares1) {
      LogService.log('✅ MATCH!');
    }
  } catch (_) {}
  ...
  for (final prefix in ppVariantPrefixes) {
    ...
    try {
      ...
      final pda = await Ed25519HDPublicKey.findProgramAddress(seeds: seeds, programId: mayflowerProgram);
      if (pda.toBase58() == expectedShares1) {
        LogService.log('✅ MATCH! Seeds: ["$prefix", marketMetadata, navMint, user]');
        LogService.log('   Result: ${pda.toBase58()}');
      }
    } catch (_) {}
  }
  ...
  for (final prefix in authPrefixes) {
    try {
      ...
      final pda = await Ed25519HDPublicKey.findProgramAddress(seeds: seeds, programId: mayflowerProgram);
      if (pda.toBase58() == expectedShares1) {
        LogService.log('✅ MATCH! Seeds: ["$prefix", authority, user]');
      }
    } catch (_) {}
    ...
    try {
      ...
      final pda = await Ed25519HDPublicKey.findProgramAddress(seeds: seeds, programId: mayflowerProgram);
      if (pda.toBase58() == expectedShares1) {
        LogService.log('✅ MATCH! Seeds: ["$prefix", user, authority]');
      }
    } catch (_) {}
    ...
    try {
      ...
      final pda = await Ed25519HDPublicKey.findProgramAddress(seeds: seeds, programId: mayflowerProgram);
      if (pda.toBase58() == expectedShares1) {
        LogService.log('✅ MATCH! Seeds: ["$prefix", authority, navMint, user]');
      }
    } catch (_) {}
  }
  ...
  try {
    ...
    LogService.log('[pp, navMint]: ${pda1.toBase58()}');
    if (pda1.toBase58() == expectedShares1) LogService.log('✅ MATCH!');
  } catch (_) {}
  ...
  try {
    ...
    LogService.log('[navMint, pp]: ${pda2.toBase58()}');
    if (pda2.toBase58() == expectedShares1) LogService.log('✅ MATCH!');
  } catch (_) {}
  ...
  try {
    ...
    final pda = await Ed25519HDPublicKey.findProgramAddress(seeds: seeds, programId: mayflowerProgram);
    LogService.log('["personal_position", marketMetadata, user, navMint]: ${pda.toBase58()}');
    if (pda.toBase58() == expectedShares1) LogService.log('✅ MATCH!');
  } catch (_) {}
  ...
  try {
    ...
    final pda = await Ed25519HDPublicKey.findProgramAddress(seeds: seeds, programId: mayflowerProgram);
    LogService.log('[marketMetadata, user, navMint]: ${pda.toBase58()}');
    if (pda.toBase58() == expectedShares1) LogService.log('✅ MATCH!');
  } catch (_) {}
  ...
  for (int i = 0; i < 10; i++) {
    try {
      ...
      final pda = await Ed25519HDPublicKey.findProgramAddress(seeds: seeds, programId: mayflowerProgram);
      if (pda.toBase58() == expectedShares1) {
        LogService.log('✅ MATCH! Seeds: [personal_position, [$i]]');
      }
    } catch (_) {}
    ...
    try {
      ...
      final pda = await Ed25519HDPublicKey.findProgramAddress(seeds: seeds, programId: mayflowerProgram);
      if (pda.toBase58() == expectedShares1) {
        LogService.log('✅ MATCH! Seeds: [[$i], personal_position]');
      }
    } catch (_) {}
  }
  ...
  for (final seedList in samsaraSeeds) {
    try {
      ...
      if (pda.toBase58() == expectedShares1) {
        LogService.log('✅ MATCH with Samsara! Seeds: $seedList');
      }
    } catch (_) {}
  }
  ...
  for (final pattern in anchorPatterns) {
    try {
      ...
      LogService.log('${pattern.value}: ${pda.toBase58()}');
      if (pda.toBase58() == expectedShares1) LogService.log('  ✅ MATCH!');
    } catch (e) {
      LogService.log('${pattern.value}: ERROR');
    }
  }
  ...
  try {
    ...
    final pda = await Ed25519HDPublicKey.findProgramAddress(seeds: seeds, programId: mayflowerProgram);
    LogService.log('["personal_position", metadata, navMint, user]: ${pda.toBase58()}');
    if (pda.toBase58() == expectedShares1) LogService.log('✅ MATCH!');
  } catch (_) {}
  ...
  try {
    ...
    final pda = await Ed25519HDPublicKey.findProgramAddress(seeds: seeds, programId: mayflowerProgram);
    LogService.log('["personal_position", navMint, metadata, user]: ${pda.toBase58()}');
    if (pda.toBase58() == expectedShares1) LogService.log('✅ MATCH!');
  } catch (_) {}
  ...
  try {
    ...
    final pda = await Ed25519HDPublicKey.findProgramAddress(seeds: seeds, programId: mayflowerProgram);
    LogService.log('[authority, navMint, user]: ${pda.toBase58()}');
    if (pda.toBase58() == expectedShares1) LogService.log('✅ MATCH!');
  } catch (_) {}
  ...
  try {
    final seeds = [user1.bytes, authority.bytes, navMint.bytes];
    final pda = await Ed25519HDPublicKey.findProgramAddress(seeds: seeds, programId: mayflowerProgram);
    LogService.log('[user, authority, navMint]: ${pda.toBase58()}');
    if (pda.toBase58() == expectedShares1) LogService.log('✅ MATCH!');
  } catch (_) {}
    ...
    for (final x in keyAccounts.entries) {
      for (final y in keyAccounts.entries) {
        ...
        try {
          ...
          final pda = await Ed25519HDPublicKey.findProgramAddress(seeds: seeds, programId: mayflowerProgram);
          if (pda.toBase58() == expectedShares1) {
            LogService.log('✅ MATCH! Mayflower: ["$prefix", ${x.key}, ${y.key}]');
          }
        } catch (_) {}
        ...
        try {
          ...
          final pda = await Ed25519HDPublicKey.findProgramAddress(seeds: seeds, programId: samsaraProgram);
          if (pda.toBase58() == expectedShares1) {
            LogService.log('✅ MATCH! Samsara: ["$prefix", ${x.key}, ${y.key}]');
          }
        } catch (_) {}
      }
    }
  ...
  for (final x in keyAccounts.entries) {
    for (final y in keyAccounts.entries) {
      ...
      try {
        ...
        final pda = await Ed25519HDPublicKey.findProgramAddress(seeds: seeds, programId: mayflowerProgram);
        if (pda.toBase58() == expectedShares1) {
          LogService.log('✅ MATCH! Mayflower no prefix: [${x.key}, ${y.key}]');
        }
      } catch (_) {}
    }
  }
  ...
  for (final combo in tripleSeeds) {
    try {
      ...
      final pda = await Ed25519HDPublicKey.findProgramAddress(seeds: seeds, programId: mayflowerProgram);
      if (pda.toBase58() == expectedShares1) {
        LogService.log('✅ MATCH! Mayflower: ["${combo[0]}", ${combo[1]}, ${combo[2]}, ${combo[3]}]');
      }
    } catch (_) {}
  }
  ...
  for (final seedStr in specificSeeds) {
    ...
    try {
      ...
      final pda = await Ed25519HDPublicKey.findProgramAddress(seeds: seeds, programId: mayflowerProgram);
      if (pda.toBase58() == expectedShares1) {
        LogService.log('✅ MATCH! ["$seedStr", marketMetadata, user]');
      }
    } catch (_) {}
    ...
    try {
      ...
      final pda = await Ed25519HDPublicKey.findProgramAddress(seeds: seeds, programId: mayflowerProgram);
      if (pda.toBase58() == expectedShares1) {
        LogService.log('✅ MATCH! ["$seedStr", user, marketMetadata]');
      }
    } catch (_) {}
    ...
    try {
      ...
      final pda = await Ed25519HDPublicKey.findProgramAddress(seeds: seeds, programId: mayflowerProgram);
      if (pda.toBase58() == expectedShares1) {
        LogService.log('✅ MATCH! ["$seedStr", navMint, user]');
      }
    } catch (_) {}
    ...
    try {
      ...
      final pda = await Ed25519HDPublicKey.findProgramAddress(seeds: seeds, programId: mayflowerProgram);
      if (pda.toBase58() == expectedShares1) {
        LogService.log('✅ MATCH! ["$seedStr", personal_position]');
      }
    } catch (_) {}
  }
  ...
  for (final seedStr in ['user_shares', 'shares', 'nav_shares', 'staked']) {
    try {
      ...
      final pda = await Ed25519HDPublicKey.findProgramAddress(seeds: seeds, programId: mayflowerProgram);
      if (pda.toBase58() == expectedShares1) {
        LogService.log('✅ MATCH! Mayflower: ["$seedStr", samsaraMarket, user]');
      }
    } catch (_) {}
    ...
    try {
      ...
      final pda = await Ed25519HDPublicKey.findProgramAddress(seeds: seeds, programId: samsaraProgram);
      if (pda.toBase58() == expectedShares1) {
        LogService.log('✅ MATCH! Samsara: ["$seedStr", samsaraMarket, user]');
      }
    } catch (_) {}
  }
  ...
  LogService.log('The user_shares address might be stored in the personal_position account data');
  LogService.log('rather than being independently derivable.');
  ...
  for (final seeds in positionPatterns) {
    try {
      ...
      if (pda.toBase58() == expectedShares1) {
        LogService.log('✅ MATCH! Mayflower: ${_describeSeed(seeds)}');
      }
    } catch (_) {}
    ...
    try {
      ...
      if (pda.toBase58() == expectedShares1) {
        LogService.log('✅ MATCH! Samsara: ${_describeSeed(seeds)}');
      }
    } catch (_) {}
  }
  ...
  for (final prefix in lastChance) {
    try {
      ...
      final pda = await Ed25519HDPublicKey.findProgramAddress(seeds: seeds, programId: mayflowerProgram);
      if (pda.toBase58() == expectedShares1) {
        LogService.log('✅ MATCH! ["$prefix", personal_position]');
      }
    } catch (_) {}
    ...
    try {
      ...
      final pda = await Ed25519HDPublicKey.findProgramAddress(seeds: seeds, programId: mayflowerProgram);
      if (pda.toBase58() == expectedShares1) {
        LogService.log('✅ MATCH! ["$prefix", marketMetadata, user]');
      }
    } catch (_) {}
  }
  ...
  LogService.log('Computed personal_position: ${computedPp.toBase58()}');
  ...
  for (final seeds in ppBasedSeeds) {
    try {
      ...
      LogService.log('  ${_describeSeed(seeds)} => ${pda.toBase58()}');
      if (pda.toBase58() == expectedShares1) {
        LogService.log('  ✅ MATCH!');
      }
    } catch (_) {}
  ...
  try {
    ...
    if (ataPda.toBase58() == expectedShares1) {
      LogService.log('  ✅ MATCH! user_shares IS a standard ATA of personal_position!');
    }
  } catch (e) {
    LogService.log('ATA error: $e');
  }
  ...
  try {
    ...
    LogService.log('[computed_pp]: ${pda.toBase58()}');
    if (pda.toBase58() == expectedShares1) LogService.log('  ✅ MATCH!');
  } catch (e) {
    LogService.log('Error: $e');
  }
  ...
  try {
    ...
    LogService.log('["b", computed_pp]: ${pda.toBase58()}');
    if (pda.toBase58() == expectedShares1) LogService.log('  ✅ MATCH!');
  } catch (_) {}
  ...
  for (final prefix in parallelPrefixes) {
    try {
      ...
      LogService.log('["$prefix", marketMetadata, user]: ${pda.toBase58()}');
      if (pda.toBase58() == expectedShares1) LogService.log('  ✅ MATCH!');
    } catch (_) {}
  }
  ...
  for (final prefix in ['shares', 'user_shares', 'nav_shares', 'staked']) {
    try {
      ...
      LogService.log('["$prefix", navMint, marketMetadata, user]: ${pda.toBase58()}');
      if (pda.toBase58() == expectedShares1) LogService.log('  ✅ MATCH!');
    } catch (_) {}
  }
  ...
  try {
    ...
    LogService.log('[pp, TOKEN_PROGRAM, navMint] with Mayflower: ${pda.toBase58()}');
    if (pda.toBase58() == expectedShares1) LogService.log('  ✅ MATCH!');
  } catch (_) {}
  ...
  try {
    ...
    LogService.log('[pp, ATA_PROGRAM, navMint] with Mayflower: ${pda.toBase58()}');
    if (pda.toBase58() == expectedShares1) LogService.log('  ✅ MATCH!');
  } catch (_) {}
  ...
  try {
    ...
    LogService.log('[authority, user, navMint]: ${pda.toBase58()}');
    if (pda.toBase58() == expectedShares1) LogService.log('  ✅ MATCH!');
  } catch (_) {}
  ...
  try {
    ...
    LogService.log('["personal_position", metadata, user, navMint]: ${pda.toBase58()}');
    if (pda.toBase58() == expectedShares1) LogService.log('  ✅ MATCH!');
  } catch (_) {}
  ...
  for (int i = 0; i < 256; i++) {
    try {
      ...
      if (pda.toBase58() == expectedShares1) {
        LogService.log('✅ MATCH! [computed_pp, [$i]]');
        break;
      }
    } catch (_) {}
    ...
    try {
      ...
      if (pda.toBase58() == expectedShares1) {
        LogService.log('✅ MATCH! [[$i], computed_pp]');
        break;
      }
    } catch (_) {}
  }
  ...
  LogService.log('\n=== Summary ===');
  LogService.log('Expected user_shares: $expectedShares1');
  LogService.log('This is a token account owned by personal_position ($expectedPosition1)');
  LogService.log('Holding navMint tokens');
  LogService.log('\n*** user_shares is stored at bytes 72-103 in personal_position account ***');
  LogService.log('For init, we may need to read from existing accounts or use a different approach.');
  ...
  try {
    ...
    if (pda.toBase58() == expectedPosition2) {
      LogService.log('✅ User2 personal_position VERIFIED with: ["personal_position", marketMetadata, user]');
      LogService.log('   Expected: $expectedPosition2');
      LogService.log('   Got: ${pda.toBase58()}\n');
    } else {
      LogService.log('❌ User2 personal_position mismatch:');
      LogService.log('   Expected: $expectedPosition2');
      LogService.log('   Got: ${pda.toBase58()}\n');
    }
  } catch (e) {
    LogService.log('Error testing user2 position: $e');
  }
