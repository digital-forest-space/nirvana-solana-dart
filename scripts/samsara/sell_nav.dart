...
void main(List<String> args) async {
  if (args.length < 2) {
    LogService.log('Usage: dart scripts/samsara/sell_nav.dart <keypair_path> <nav_amount> [--rpc <url>] [--verbose]');
    LogService.log('');
  ...
  if (navAmount == null || navAmount <= 0) {
    LogService.log(jsonEncode({'success': false, 'error': 'Invalid navSOL amount: ${args[1]}'}));
    exit(1);
  }
  ...
  if (verbose) {
    LogService.log('\nTransaction:');
    LogService.log('  Input: $navAmount navSOL ($navLamports lamports)');
