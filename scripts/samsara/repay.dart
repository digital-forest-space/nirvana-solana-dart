...
/// Repay borrowed base token to a Mayflower market
///
/// Usage: dart scripts/samsara/repay.dart <keypair_path> <amount> [--market <name>] [--rpc <url>] [--verbose] [--dry-run]
///
/// Examples:
///   dart scripts/samsara/repay.dart ~/.config/solana/id.json 0.01
///   dart scripts/samsara/repay.dart ~/.config/solana/id.json 0.5 --market navSOL --verbose
///   dart scripts/samsara/repay.dart ~/.config/solana/id.json 0.001 --market navCBBTC --dry-run
///
/// Environment:
...
void main(List<String> args) async {
  if (args.length < 2) {
    LogService.log('Usage: dart scripts/samsara/repay.dart <keypair_path> <amount> [options]');
    LogService.log('');
    LogService.log('Repays borrowed base token (e.g., SOL) to a navToken market.');
    LogService.log('');
    LogService.log('Options:');
    LogService.log('  --market <name>  Market to repay to (default: navSOL)');
  ...
  if (verbose) {
    LogService.log('\nRepay:');
    LogService.log('  Amount: $amount ${market.baseName} ($lamports lamports)');
  ...
  final unsignedTxBytes = await client.buildUnsignedRepayTransaction(
    userPubkey: userPubkey,
    market: market,
    repayLamports: lamports,
