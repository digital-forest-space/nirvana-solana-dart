...
void main() async {
  ...
  LogService.log('Decoding personal_position account: $positionAddress\n');
  ...
  try {
    ...
    if (result['result'] != null && result['result']['value'] != null) {
      ...
      LogService.log('Account data length: ${data.length} bytes');
      LogService.log('Owner: ${value['owner']}');
      LogService.log('\n=== Raw data (hex) ===');
      LogService.log(data.map((b) => b.toRadixString(16).padLeft(2, '0')).join(' '));
        ...
        for (int offset = 8; offset <= data.length - 32; offset++) {
          try {
            ...
            if (!_isAllZeros(pubkeyBytes) && !_isAllOnes(pubkeyBytes)) {
              LogService.log('Offset $offset: $address');
              if (address == expectedShares) {
                LogService.log('  ^^^ THIS IS THE EXPECTED user_shares! ^^^');
              }
            }
          } catch (e) {
            // Not a valid pubkey
      ...
      if (data.length >= 8) {
        ...
        LogService.log('Discriminator: ${discriminator.map((b) => b.toRadixString(16).padLeft(2, '0')).join(' ')}');
      }
      ...
      if (data.length >= 8 + 32) {
        ...
        if (field1.toBase58() == expectedShares) LogService.log('  ^^^ MATCH user_shares! ^^^');
      }

      if (data.length >= 8 + 64) {
        final field2 = Ed25519HDPublicKey(data.sublist(40, 72));
        LogService.log('Field 2 (bytes 40-71): ${field2.toBase58()}');
        if (field2.toBase58() == expectedShares) LogService.log('  ^^^ MATCH user_shares! ^^^');
      }

      if (data.length >= 8 + 96) {
        final field3 = Ed25519HDPublicKey(data.sublist(72, 104));
        LogService.log('Field 3 (bytes 72-103): ${field3.toBase58()}');
        if (field3.toBase58() == expectedShares) LogService.log('  ^^^ MATCH user_shares! ^^^');
      }

      if (data.length >= 8 + 128) {
        final field4 = Ed25519HDPublicKey(data.sublist(104, 136));
        LogService.log('Field 4 (bytes 104-135): ${field4.toBase58()}');
        if (field4.toBase58() == expectedShares) LogService.log('  ^^^ MATCH user_shares! ^^^');
      }
      ...
      if (data.length >= 144) {
        ...
        if (remaining.length >= 8) {
          ...
          for (int i = 0; i + 8 <= remaining.length; i += 8) {
            ...
            LogService.log('  Offset ${136 + i}: $val');
          }
        }
      }
    } else {
