...
void main(List<String> args) async {
  if (args.length < 2) {
    LogService.log('Usage: dart scripts/sell_ana.dart <keypair_path> <ana_amount> [--nirv|--usdc] [--rpc <url>] [--verbose]');
    LogService.log('');
    LogService.log('Options:');
    LogService.log('  --nirv       Receive NIRV instead of USDC');
    LogService.log('  --usdc       Receive USDC (default)');
  ...
  if (verbose) {
    LogService.log('\nTransaction:');
    LogService.log('  Selling: $anaAmount ANA');
    LogService.log('  Receive: $receiveCurrency');
    LogService.log('  Estimated: ${estimatedOutput.toStringAsFixed(6)} $receiveCurrency (after ~3% fee)');
